...
import ImportContactsReviewTab from './import-contacts-tabs/ImportContactsReviewTab';
import ImportContactsPaymentTab from './import-contacts-tabs/ImportContactsPaymentTab';
import ImportContactsCompleteTab from './import-contacts-tabs/ImportContactsCompleteTab';
    ...
    <>
      ...
      <div className='py-20 sm:py-30 px-15 sm:px-30 bg-white rounded-15px sm:rounded-20px'>
        ...
        <div className='flex justify-between items-center flex-wrap gap-10 w-full'>
          <NavLink to={"/contacts"} className='flex items-center justify-start gap-10'>
            <IoChevronBack className='text-xl text-main-text' />
            <h2 className='font-semibold text-17px sm:text-xl text-main-text'>Import Contacts</h2>
          </NavLink>
          ...
          <div className="flex justify-end  gap-10 sm:gap-25 flex-wrap">
            ...
            <div className='flex items-center justify-end gap-5 sm:gap-10'>
              ...
              {/* disable btn */}
              {/* <NavLink to={"/create-labels"} className='flex items-center justify-between gap-8 disable-button-icon'>
                <span className='sm:block hidden'> Next</span>
                <IoChevronForward className='text-15px text-white' />
              </NavLink> */}
            </div>
      ...
      {/* tab content */}
      {activeTab === "review" && <ImportContactsReviewTab />}
      {activeTab === "payment" && <ImportContactsPaymentTab />}
      {activeTab === "complete" && <ImportContactsCompleteTab />}
